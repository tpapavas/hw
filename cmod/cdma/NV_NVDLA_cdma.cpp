    ...
    b_transport_delay_(SC_ZERO_TIME)
{
    ...
    act_data_read_rsp_fifo_ = new sc_core::sc_fifo <uint8_t*> (MAX_MEM_TRANSACTION_SIZE*10/ATOM_CUBE_SIZE);
    mean_data_read_rsp_fifo_ = new sc_core::sc_fifo <uint8_t*> (MAX_MEM_TRANSACTION_SIZE/ATOM_CUBE_SIZE);
    weight_read_rsp_fifo_   = new sc_core::sc_fifo <uint8_t*> (MAX_MEM_TRANSACTION_SIZE/ATOM_CUBE_SIZE);
    wmb_read_rsp_fifo_ = new sc_core::sc_fifo <uint8_t*> (MAX_MEM_TRANSACTION_SIZE/ATOM_CUBE_SIZE);
    wgs_read_rsp_fifo_ = new sc_core::sc_fifo <uint8_t*> (MAX_MEM_TRANSACTION_SIZE/ATOM_CUBE_SIZE);
...
NV_NVDLA_cdma::~NV_NVDLA_cdma() {
    ...
    if( dma_wgs_rd_req_payload_ )   delete dma_wgs_rd_req_payload_;
    if( dma_wmb_rd_req_payload_ )   delete dma_wmb_rd_req_payload_;
...
void NV_NVDLA_cdma::Reset()
{
    ...
    cdma_req_prev_data_bank_        = CBUF_BANK_NUM + 1;
...
void NV_NVDLA_cdma::WeightReadRequestSequenceThread () {
    ...
    while (true) {
        ...
        if (wt_first_layer) {  // The first layer
            ...
            weight_entry_idx_free_   = (cdma_weight_bank_ + 1) * CBUF_ENTRY_PER_BANK - 1;
            weight_entry_idx_working_ = 0;
            weight_entry_idx_planed_ = -1;
            weight_byte_idx_planed_  = 0;
...
void NV_NVDLA_cdma::WGSReadRequestSequenceThread () {
    ...
    while (true) {
        ...
        if(cdma_weight_format_ == NVDLA_CDMA_D_WEIGHT_FORMAT_0_WEIGHT_FORMAT_COMPRESSED) {
            ...
            if(wgs_buffer_)
                delete[] wgs_buffer_;
            wgs_buffer_         = new uint32_t[(((wgs_total_bytes+31)/32) * 32)/4]; // Align to 32B
            if (cdma_wt_dma_arbiter_override_enable) {
                for(uint32_t i=0;i<(((wgs_total_bytes+31)/32) * 32)/4;i++)
...
void NV_NVDLA_cdma::DirectConvDataRequestSequencerCommon() {
    ...
    bool     is_surf_packed;
    // Temp variables
    uint32_t atom_num[2], atom_sent_num[2];
    uint64_t payload_addr[2];
    ...
    base_addr           = (uint64_t(cdma_datain_addr_high_0_) << 32) + (uint64_t(cdma_datain_addr_low_0_) << 5);
    line_stride         = cdma_line_stride_ << 5;
    surface_stride      = cdma_surf_stride_ << 5;
    batch_num           = cdma_batches_ + 1;
    batch_stride        = cdma_batch_stride_ << 5;
...
#pragma CTC SKIP
    if ((batch_num > 1) && (batch_stride < (surface_stride * ((cube_channel + element_per_atom - 1) / element_per_atom))))
...
void NV_NVDLA_cdma::DirectConvDataResponseSequencerCommon() {
    ...
    int32_t  cbuf_entry_per_slice;
    //uint32_t super_atom_fetched[4]; // The number of packed 64B cbuf half entries
    //uint32_t super_atom_stored; // The number of packed 64B cbuf half entries
    ...
    bool     last_super_surface;
    uint32_t last_super_surface_size;
    uint8_t  fetched_data_1x1[CBUF_ENTRY_SIZE/2], converted_fetched_data_1x1[CBUF_ENTRY_SIZE/2];
...
#pragma CTC ENDSKIP
    ...
    atom_per_channel        = (cube_channel*element_size + ATOM_CUBE_SIZE - 1) / ATOM_CUBE_SIZE;
    cbuf_entry_per_slice    = (atom_per_channel / 4) * cube_width;
    ...
    if ((cdma_entries_ + 1) != cbuf_entry_per_slice) {
    ...
    is_surf_packed  = cdma_surf_packed_;
#pragma CTC SKIP
    if (is_line_packed != (line_stride == ATOM_CUBE_SIZE * cube_width)) {
        FAIL(("NV_NVDLA_cdma::DirectConvDataResponseSequencerCommon, invalid configuration of cdma_line_packed_, cdma_line_packed_ is %d, line_stride == ATOM_CUBE_SIZE * cube_width is %d.", cdma_line_packed_, (line_stride == ATOM_CUBE_SIZE * cube_width)));
    }
...
#pragma CTC ENDSKIP
        ...
        for (batch_iter=0; batch_iter<batch_num; batch_iter++) {
            ...
            while (read_atom_num_1x1 < atom_num_1x1) {
                ...
                while ( read_atom_iter_1x1 < super_normal_ratio ) {
                    ...
                    memcpy(&fetched_data_1x1[read_atom_iter_1x1*ATOM_CUBE_SIZE], read_data_ptr, ATOM_CUBE_SIZE);
                ...
                cbuf_entry_addr = (data_entry_idx_working_ + batch_iter * cbuf_entry_per_slice + (read_atom_num_1x1-1)/(2*super_normal_ratio)) % data_entry_num;
                cdma2cbuf_data_payload_->hsel = 1 - cdma2cbuf_data_payload_->hsel;
            ...
            data2sc_data_update_payload_->dat_entries = (atom_num_1x1+3)/4;
            ...
            for (batch_iter=0; batch_iter<batch_num; batch_iter++) {
                for (super_surface_iter = 0; super_surface_iter<super_surface_num; super_surface_iter ++) {
                    ...
                    for (payload_atom_iter = 0; payload_atom_iter < cube_width * current_slice_grain; payload_atom_iter++) {
                        super_surf_fetched_data_ptr[payload_atom_iter] = new uint8_t[CBUF_HALF_ENTRY_SIZE];
                        converted_super_surf_fetched_data_ptr[payload_atom_iter] = new uint8_t[CBUF_HALF_ENTRY_SIZE];
                    }
                    while ( (atom_sent_num[0] < atom_num[0]) || (atom_sent_num[1] < atom_num[1]) ) {
                        ...
                        for (surface_iter=0; surface_iter<super_normal_ratio; surface_iter++) {
                            ...
                            if (atom_sent_num[surface_iter] < atom_num[surface_iter]) {
                                ...
                                for (payload_atom_iter=0; payload_atom_iter<payload_atom_num; payload_atom_iter++) {
                                    ...
                                    memcpy (&super_surf_fetched_data_ptr[curr_payload_idx][surface_iter*ATOM_CUBE_SIZE], read_data_ptr, ATOM_CUBE_SIZE);
                                    delete [] read_data_ptr;
                                }
                                payload_addr[surface_iter]  += payload_size;
                                atom_sent_num[surface_iter] += payload_atom_num;
                    ...
                    if (cvt_en) {
                        switch (precision) {
                            case DATA_FORMAT_INT8:
                                for (i=0;i<cube_width * current_slice_grain;i++) {
                                    for (j=0;j<CBUF_HALF_ENTRY_SIZE;j++) {
                                        int16_t convert_input;
                                        int32_t convert_result;
                    ...
                    for (height_iter=0; height_iter<current_slice_grain; height_iter++) {   // height iterator in current super slice
                        ...
                        while(atom_stored < cube_width) {
                            if (last_super_surface && (super_surface_num % 2 == 1)) {   // The number of super surface is an odd number (pack the data in last super surface into one cbuf entry)
                                ...
                                memcpy(cbuf_payload_data_ptr, fetched_data_ptr[height_iter*cube_width+atom_stored], ATOM_CUBE_SIZE*2);
                                cbuf_entry_addr = (data_entry_idx_working_ + ((super_slice_iter * fetch_slice_grain + height_iter) * batch_num + batch_iter ) * cbuf_entry_per_slice + (super_surface_iter/2) * cube_width + atom_stored % cube_width) % data_entry_num;
                                if (super_surface_iter%2 == 0) {
                                    cdma2cbuf_data_payload_->hsel = 0;
                                } else {
                                    cdma2cbuf_data_payload_->hsel = 1;
                                }
    ...
    //gNvdlaStats.cbufDataWriteBytes += payload_size;
}
...
void NV_NVDLA_cdma::ConvWGSRequestSequencerCommon() {
    ...
    // Copy from register value to local config variables, similar with RTL connection
    base_addr           = (uint64_t(cdma_wgs_addr_high_) << 32) + (uint64_t(cdma_wgs_addr_low_) << 5);
    weight_kernel       = cdma_weight_kernel_ + 1;
    proc_precision      = cdma_proc_precision_;
    switch (proc_precision) {
        case DATA_FORMAT_INT8:
            wgs_total_bytes     = (weight_kernel + KERNEL_PER_GROUP_INT8 - 1) / KERNEL_PER_GROUP_INT8 * 4;
            break;
        case DATA_FORMAT_INT16:
        case DATA_FORMAT_FP16:
...
void NV_NVDLA_cdma::ConvWMBRequestSequencerCommon() {
    ...
    uint32_t kernel_group_wmb_size;

    // Copy from register value to local config variables, similar with RTL connection
    base_addr           = (uint64_t(cdma_wmb_addr_high_) << 32) + (uint64_t(cdma_wmb_addr_low_) << 5);
    wmb_total_bytes     = cdma_wmb_bytes_ << 7;
    proc_precision      = cdma_proc_precision_;
    switch (proc_precision) {
        case DATA_FORMAT_INT8:
    ...
    while (wmb_bytes_fetched < wmb_total_bytes) {
        ...
        payload_addr            += payload_size;
        // Total weight byte index is 128 aligned
        // When a kernel group of WMB is done and in scve environment
//        if ((NVDLA_CDMA_D_WEIGHT_FORMAT_0_WEIGHT_FORMAT_COMPRESSED == cdma_weight_format_) && !cdma_wt_dma_arbiter_override_enable) {
//            cslDebug((50, "NV_NVDLA_cdma::ConvWMBRequestSequencerCommon wmb_bytes_fetched=0x%x\n", wmb_bytes_fetched));
//            if ((wmb_bytes_fetched >= wmb_total_bytes) || ((wmb_bytes_fetched - prev_wmb_bytes_fetched) >= kernel_group_wmb_size)) {
//                prev_wmb_bytes_fetched = wmb_bytes_fetched;
//                // wait wt finish reading a kernel group
//                cslDebug((50, "write wt2wmb_kg_sync_fifo. wmb_bytes_fetched=0x%x\n", wmb_bytes_fetched));
//                wt2wmb_kg_sync_fifo_->read();
//                cslDebug((50, "write wt2wmb_kg_sync_fifo done\n"));
//            }
//        }
    }
...
void NV_NVDLA_cdma::ConvWMBResponseSequencerCommon() {
    ...
    uint32_t kernel_per_group, current_kernel_per_group;
    ...
    while (wmb_bytes_fetched < wmb_total_bytes) {
        ...
        if ( 0 == (wmb_bytes_fetched % CBUF_HALF_ENTRY_SIZE) ) {
            ...
            if ( (wmb_bytes_fetched % CBUF_ENTRY_SIZE) == 64 )
                cdma2buf_wmb_wr_payload.hsel = 0;
            else {
                cdma2buf_wmb_wr_payload.hsel = 1;
    ...
    if (wmb_entries_fetched_!=(wmb_total_bytes/CBUF_ENTRY_SIZE))
        FAIL(("The total size of weight should be multiple of CBUF_ENTRY_SIZE\n"));
}
...
void NV_NVDLA_cdma::DirectConvWeightRequestSequencerCommon(){
    ...
    // Copy from register value to local config variables, similar with RTL connection
    base_addr           = (uint64_t(cdma_weight_addr_high_) << 32) + (uint64_t(cdma_weight_addr_low_) << 5);
    weight_total_bytes  = cdma_weight_bytes_ << 7;
    ...
    wt_required_entries_num = (weight_total_bytes + CBUF_ENTRY_SIZE - 1) / CBUF_ENTRY_SIZE;
    if ((wt_required_entries_num > (cdma_weight_bank_ + 1) * CBUF_ENTRY_PER_BANK) && cdma_skip_weight_rls_) {
        FAIL (("NV_NVDLA_cdma::DirectConvWeightRequestSequencerCommon, invalid config, The total weight size is larger than cdma_weight_bank_*CBUF_ENTRY_SIZE, but cdma_skip_weight_rls_ is true."));
    }
...
void NV_NVDLA_cdma::DirectConvWeightResponseSequencerCommon() {
    ...
    // Copy from register value to local config variables, similar with RTL connection
    weight_entry_addr_start     = (cdma_data_bank_+1) * CBUF_ENTRY_PER_BANK;
    weight_entry_addr_aperture  = (cdma_weight_bank_+1) * CBUF_ENTRY_PER_BANK;
    byte_per_kernel             = cdma_byte_per_kernel_ + 1;
    weight_total_bytes          = cdma_weight_bytes_ << 7;
    proc_precision              = cdma_proc_precision_;
    weight_kernel               = cdma_weight_kernel_ + 1;
    ...
    while (weight_bytes_fetched < weight_total_bytes) {
        ...
        if ( 0 == (weight_bytes_fetched % CBUF_HALF_ENTRY_SIZE) ) {
            ...
            if ((weight_bytes_fetched % CBUF_ENTRY_SIZE) == 64)
                cdma2buf_wt_wr_payload.hsel = 0;
            else {
                cdma2buf_wt_wr_payload.hsel = 1;
                wt_entries_fetched++;
            }
            // No NaN and Inf in Weight. But still count them to match RTL.
            if (DATA_FORMAT_FP16 == proc_precision) {
                for(idx=0;idx<CBUF_HALF_ENTRY_SIZE/2;idx++) {
    ...
    if (wt_entries_fetched!=(weight_total_bytes/CBUF_ENTRY_SIZE))
        FAIL(("The total size of weight should be multiple of CBUF_ENTRY_SIZE\n"));
    //gNvdlaStats.cbufDataWriteBytes += weight_total_bytes;
}
...
void NV_NVDLA_cdma::ImageConvDataRequestSequencerCommon() {
    ...
    cube_height         = cdma_datain_height_ + 1;
    line_stride         = cdma_line_stride_ << 5;
    uv_line_stride      = cdma_uv_line_stride_ << 5;
    cube_in_channel     = cdma_datain_channel_ + 1;
    cbuf_entry_per_line = cdma_entries_ + 1;

    planar0_base_addr   = (uint64_t(cdma_datain_addr_high_0_) << 32) + (uint64_t(cdma_datain_addr_low_0_) << 5);
    planar1_base_addr   = (uint64_t(cdma_datain_addr_high_1_) << 32) + (uint64_t(cdma_datain_addr_low_1_) << 5);
    precision           = cdma_in_precision_;
    ...
    element_size        = packed_format(pixel_format)? 1: (DATA_FORMAT_INT8 == precision)? 1: 2;

    if (1==planar_num) {
        p0_pad_left_main = int(pad_left * cube_in_channel * element_size / 32);
        p0_pad_left_tail = (pad_left * cube_in_channel * element_size) % 32;
        ...
        p0_pad_left_main = int(pad_left * element_size / 32);
        p0_pad_left_tail = (pad_left * element_size) % 32;
        p0_x_offset_byte = pixel_x_offset * element_size;
        p0_line_st_minus = (p0_pad_left_tail > p0_x_offset_byte) ? p0_pad_left_main + 1 : p0_pad_left_main;

        p1_pad_left_main = int(pad_left * 2 * element_size / 32);
        p1_pad_left_tail = (pad_left * 2 * element_size) % 32;
        p1_x_offset_byte = (pixel_x_offset * 2 * element_size) % 32;
    ...
    if (pitch_linear) {
        ...
        planar1_to_fetch_bytes = ((pixel_x_offset * bytes_per_pixel_p1) % ATOM_CUBE_SIZE) + cube_width * bytes_per_pixel_p1;
...
void NV_NVDLA_cdma::ImageConvDataResponseSequencerCommon() {
    ...
    uint8_t** pad_buffer;
    uint8_t to_cbuf_128B[128];
    uint8_t zero_array[128] = {0};
    ...
    for (i=0; i < cube_height; i++) {
        pad_buffer[i] = new uint8_t[pad_line_bytes];
    }
    ...
    if (pitch_linear) {
        ...
        uint32_t planar1_to_fetch_bytes = ((pixel_x_offset * bytes_per_pixel_p1) % ATOM_CUBE_SIZE) + cube_width * bytes_per_pixel_p1;
        uint32_t pitch_line_bytes_rounded = ((pixel_x_offset + cube_width) * bytes_per_pixel + ATOM_CUBE_SIZE - 1)/ATOM_CUBE_SIZE * ATOM_CUBE_SIZE;
        p0_line_st = 9 - p0_line_st_minus;          //For pitch linear, unit is burst
        p1_line_st = 17 - p1_line_st_minus;         //for pitch linear, unit is burst 
        ...
        for (height_iter=0; height_iter < cube_height; height_iter++) {
            pitch_buffer[height_iter] = new uint8_t[pitch_line_bytes_rounded];
        }
        ...
        for (height_iter=0; height_iter<cube_height; height_iter++) {     // read data line by line and assemble into pitch_buffer
            ...
            if ((pixel_x_offset * 2 * element_size) >= ATOM_CUBE_SIZE)
            ...
            while ((planar0_bytes_fetched < planar0_to_fetch_bytes) || ((2==planar_num) && (planar1_bytes_fetched < planar1_to_fetch_bytes))) {
                ...
                if (planar0_bytes_fetched < planar0_to_fetch_bytes) {
                    ...
                    planar0_bytes_fetched += payload_atom_num * ATOM_CUBE_SIZE;
                }
                ...
                if ((2==planar_num) && (planar1_bytes_fetched < planar1_to_fetch_bytes)) {
                    ...
                    planar1_bytes_fetched += payload_atom_num * ATOM_CUBE_SIZE;
                }
            }
        }
        ...
        if (DATA_FORMAT_FP16 == precision) {
            for (height_iter=0; height_iter<cube_height; height_iter++) {     // read data line by line and assemble into pad_buffer
                int32_t element_iter = pixel_x_offset * bytes_per_pixel / element_size; // The NaN and Inf in pixel_x_offset range are not counted.
    ...
    for (height_iter=0; height_iter<cube_height; height_iter++) {     // read data line by line and assemble into pad_buffer
        ...
        while(pixel_idx < (pad_left+cube_width)) {
        ...
        for (;pixel_idx<pad_left+cube_width+pad_right; pixel_idx++) {    // pad_left is in pixel, not byte
            for (element_idx=0; element_idx < element_num; element_idx++) {
                if (DATA_FORMAT_INT8 == proc_precision)
                    memcpy(&pad_buffer_8[pixel_idx*element_num + element_idx], &pad_value_8, sizeof(uint8_t));
                else
    ...
    for (height_iter=0; height_iter<cube_height; height_iter++) {
        ...
        while (write_line_bytes < pad_line_bytes) {
            ...
            if (curr_cbuf_entry_size == CBUF_ENTRY_SIZE)
                ...
                memcpy(to_cbuf_128B, &pad_buffer[height_iter][write_line_bytes], curr_cbuf_entry_size);
                memcpy(&to_cbuf_128B[curr_cbuf_entry_size], zero_array, CBUF_ENTRY_SIZE - curr_cbuf_entry_size);
            }
            // Write one entry to CBUF
            WriteOneEntryToCbuf(to_cbuf_128B, cbuf_entry_addr);
            cbuf_entry_addr = (cbuf_entry_addr+1) % ((cdma_data_bank_+1) * CBUF_ENTRY_PER_BANK);
    ...
    for (super_height_iter=0; super_height_iter<super_height_num; super_height_iter++) {
        ...
        for (surface_iter= 0; surface_iter<surface_num; surface_iter++) {
            ...
            for (stride_y_iter=0; stride_y_iter < conv_y_stride; stride_y_iter++) {
                ...
                for (width_iter= 0; width_iter<cube_width; width_iter=width_iter+8) { // The left pad and right pad are not fetched. The last transaction may be less than 8 atoms.
...
void NV_NVDLA_cdma::WinoConvDataResponseSequencerCommon() {
    ...
    uint32_t cbuf_entry_addr;
    uint32_t data_entry_num;
    //uint8_t  atom_pad_value_int8[ATOM_CUBE_SIZE];
    uint16_t atom_pad_value_int16[ATOM_CUBE_SIZE/2];

    // Temp variables
    uint8_t  read_data_ptr[4][READ_WINO_BUF_WIDTH][ATOM_CUBE_SIZE];
    uint8_t  cbuf_entry_buffer[128];
...
#pragma CTC ENDSKIP
    ...
    // super_surface_stride    = surface_stride * super_normal_ratio;
    //element_per_super_atom  = element_per_atom * super_normal_ratio;
    //super_slice_num         = (cube_height+fetch_slice_grain-1)/fetch_slice_grain;
    surface_num             = (cube_channel + element_per_atom - 1)/element_per_atom;
...
#pragma CTC SKIP
    ...
    if ((cdma_entries_+ 1) != cbuf_entry_per_slice/4) {
        FAIL(("NV_NVDLA_cdma::DirectConvDataResponseSequencerCommon, invalid configuration cdma_entries_, actual value is 0x%X, it shall be 0x%X.", cdma_entries_, cbuf_entry_per_slice-1));
    }
    is_line_packed  = cdma_line_packed_;
...
#pragma CTC ENDSKIP
                    ...
                    for (x =0;x <4*conv_x_stride;x++) {
                        for (i=0; i<4; i++) {  // 4 rows
                            ...
                            if ((height_iter < pad_top) || (height_iter >= (pad_top+cube_height)) ||
                                ...
                                // Padding value
                                memcpy(read_data_ptr[i][x], atom_pad_value_int16, ATOM_CUBE_SIZE);
                            }
                            else {
                                read_atom_ptr = wino_fetch_data_fifo_[i]->read();
                                memcpy(read_data_ptr[i][x], read_atom_ptr, ATOM_CUBE_SIZE);
                    ...
                    for (i=0; i<conv_x_stride; i++) {       // Number pf stride_x 4x4x4 sub-cubes in width direction
                        for (j=0; j<4; j++) {               // 4 4x4x4 sub-cubes in channel diretion
                            ...
                            //if (flush_nan2zero&&(DATA_FORMAT_FP16 == precision)) {
                            if (DATA_FORMAT_FP16 == precision) {
                                for(k=0;k<CBUF_HALF_ENTRY_SIZE;k++) {
                            ...
                            coor_channel_4x4x4 = ((stride_y_iter * conv_x_stride + i) * surface_num + surface_iter)*4 + j;
                            ...
                            //cbuf_entry_addr    = (coor_height_4x4x4 * (cube_width_ext/4) + coor_width_4x4x4) * num_4x4x4_channel + coor_channel_4x4x4;
...
void NV_NVDLA_cdma::WriteOneEntryToCbuf(uint8_t* read_data_ptr, uint32_t cbuf_entry_addr)
{
