...
void NV_NVDLA_cacc::CaccHardwareLayerExecutionTrigger () {
    ...
    if(first_layer) {
        ...
        if (DATA_FORMAT_INT8==cacc_proc_precision_) {
            assembly_sram_group_idx_available_  = -2 * atom_per_mac_cell;  //assembly_sram_group_idx_available_ points to the entry which has valid data to process
            assembly_sram_group_idx_fetched_    = -2 * atom_per_mac_cell; //assembly_sram_group_idx_fetched_ points to the last entry which has been processed
            delivery_sram_group_idx_available_  = -2 * atom_per_mac_cell;
            delivery_sram_group_idx_fetched_    = -2 * atom_per_mac_cell;
        }
        else {
...
void NV_NVDLA_cacc::SendToSDPCommon () {
    ...
    base_addr    = cacc_config->cacc_dataout_addr_ << 5;
    line_stride  = cacc_config->cacc_line_stride_ << 5;
    ...
    switch (precision) {
        case DATA_FORMAT_INT8:
            element_per_atom = ATOM_ELEMENT_NUM_INT8;
            //round_stride_accu = 2*MAC_CELL_NUM;
            trans_num = 2;
            break;
        case DATA_FORMAT_INT16:
            element_per_atom = ATOM_ELEMENT_NUM_INT16;
            //round_stride_accu = MAC_CELL_NUM;
            trans_num = 1;
            break;
        case DATA_FORMAT_FP16:
            element_per_atom = ATOM_ELEMENT_NUM_FP16;
            //round_stride_accu = MAC_CELL_NUM;
            trans_num = 1;
            break;
    }
    ...
    atom_num = uint64_t(cube_width) * cube_height * ((cube_channel + element_per_atom - 1) / element_per_atom);
    batch_atom_num = batch_num * atom_num;
    element_per_payload = CACC_TO_SDP_THROUGHPUT_INT8;  // CACC_TO_SDP_THROUGHPUT_INT8 is 16
    atom_num_sent = 0;
    ...
    while (atom_num_sent < batch_atom_num) {
        ...
        for(i=0;i<batch_num;i++) {
            if (atom_num_batch_2sdp==batch_num) {
                for(int atom_iter = 0; atom_iter < atom_per_mac_cell; atom_iter++) {
                ...
                for (trans_iter=0;trans_iter<trans_num;trans_iter++) {
                    for(j=0;j<element_per_payload;j++) {
                        payload_data_ptr[j] = to_sdp_atom[i*trans_num+trans_iter][j];
                    }
                ...
                for (trans_iter=0;trans_iter<trans_num;trans_iter++) {
                    for(j=0;j<element_per_payload;j++) {
                        payload_data_ptr[j] = to_sdp_atom[(i+batch_num)*trans_num+trans_iter][j];
                    }
                    delete [] to_sdp_atom[(i+batch_num)*trans_num+trans_iter];
                    if ((trans_iter==trans_num-1) && (atom_num_sent + 1 == batch_atom_num)) {    // Send layer_end to SDP only for the last cube in the batch
                        cacc2sdp_payload.pd.nvdla_cc2pp_pkg.batch_end = 0;
                        cacc2sdp_payload.pd.nvdla_cc2pp_pkg.layer_end = 1;
                    }
...
void NV_NVDLA_cacc::DeliverSequencerDirectConvCommon() {
    ...
    while (atom_num_sent < batch_atom_num) {
        ...
        if (delivery_first_layer) {     // first layer and first atom
            ...
            if (DATA_FORMAT_INT8 == deliver_prev_precision_) {
                if (deliver_prev_conv_mode_ == CONV_MODE_WINOGRAD) {
                    delivery_sram_group_idx_fetched_ += 8;
                } else {
                    delivery_sram_group_idx_fetched_ += 2;
                }
            } else {
                if (deliver_prev_conv_mode_ == CONV_MODE_WINOGRAD) {
            ...
            case DATA_FORMAT_INT8:
                ...
                for(atom_per_mac_cell_iter=0;atom_per_mac_cell_iter<atom_per_mac_cell;atom_per_mac_cell_iter++) {
                    ...
                    for (mac_cell_iter = 0; mac_cell_iter < MAC_CELL_NUM; mac_cell_iter ++) {
                        ...
                        prepared_sdp_atom[element_per_payload_prepared%element_per_payload] = delivery_sram_group_[((delivery_sram_group_idx_fetched_+atom_per_mac_cell_iter*2)*round_stride_accu+mac_cell_iter*2+1)%SRAM_GROUP_SIZE];
                        element_per_payload_prepared += 1;
        ...
        if (DATA_FORMAT_INT8==precision)
            accu2sc_credit_payload.size = 2*atom_per_mac_cell;
        else
...
void NV_NVDLA_cacc::ReshapeSequencerDirectConvCommon() {
    ...
    channel_num = ((cube_channel + element_per_atom - 1) / element_per_atom) * element_per_atom;
    atom_num = uint64_t(cube_width) * cube_height * channel_num / element_per_atom;
    batch_atom_num = batch_num * atom_num;
    ...
    while (atom_num_sent < batch_atom_num) {
        ...
        if (reshape_first_layer) {  // first layer and first atom
            ...
            if (DATA_FORMAT_INT8 == precision) {
                assembly_sram_group_idx_fetched_    += 2*atom_per_mac_cell;
                delivery_sram_group_idx_available_  += 2*atom_per_mac_cell;
            }
            else {
            ...
            if (DATA_FORMAT_INT8 == reshape_prev_precision_) {
                if (reshape_prev_conv_mode_ == CONV_MODE_WINOGRAD) {
                    assembly_sram_group_idx_fetched_    += 8;
                    delivery_sram_group_idx_available_  += 8;
                } else {
                    assembly_sram_group_idx_fetched_    += 2;
                    delivery_sram_group_idx_available_  += 2;
                }
            } else {
                if (reshape_prev_conv_mode_ == CONV_MODE_WINOGRAD) {
        ...
        for (mac_cell_iter = 0; mac_cell_iter < MAC_CELL_NUM; mac_cell_iter ++) {
            switch (precision) {
                case DATA_FORMAT_INT8:
                    // For INT8, 48bits is also used for the assembly group
                    // The result is 32x32bits (to sdp). In RTL, 2 cycles are used to transfer them to sdp.
                    for(atom_per_mac_cell_iter=0;atom_per_mac_cell_iter<atom_per_mac_cell;atom_per_mac_cell_iter++) {
                        for (kernel_iter = 0; kernel_iter < 2; kernel_iter++) {
                            assembly_idx = ((assembly_sram_group_idx_fetched_+atom_per_mac_cell_iter*2)*round_stride_accu+mac_cell_iter*2+kernel_iter)%SRAM_GROUP_SIZE;
                            delivery_idx = ((delivery_sram_group_idx_available_+atom_per_mac_cell_iter*2)*round_stride_accu+mac_cell_iter*2+kernel_iter)%SRAM_GROUP_SIZE;
...
void NV_NVDLA_cacc::mac2accu_b_transport(nvdla_mac2accu_data_concat_if_t* payload, sc_time& delay) {
    ...
    // Evaluated
    // round_stride_accu contains an atom to output to SDP
    switch (precision) {
        case DATA_FORMAT_INT8:
            round_stride_accu = MAC_CELL_NUM;
            break;
    ...
    if (false == has_ongoing_channel_operation_) {
        ...
        for (mac_cell_iter = 0; mac_cell_iter < MAC_CELL_NUM; mac_cell_iter ++) {
            for (atom_iter=0; atom_iter<atom_per_mac_cell; atom_iter++) {
                switch (precision) {
                    case DATA_FORMAT_INT8:
                        payload_idx = mac_cell_iter * ELEMENT_PER_MAC_CELL_INT8 + atom_iter * 2;
                        assembly_idx = ((assembly_sram_group_idx_working_+atom_iter*2)*round_stride_accu+mac_cell_iter*2)%SRAM_GROUP_SIZE;
                        ...
                        payload_idx ++;
                        assembly_idx = ((assembly_sram_group_idx_working_+atom_iter*2)*round_stride_accu+mac_cell_iter*2+1)%SRAM_GROUP_SIZE;
                        int8_mac_data  = payload_data_ptr[payload_idx];
                        assembly_sram_group_[assembly_idx] = int8_mac_data;
#if LOG_DETAIL
                        ...
                        break;
                    case DATA_FORMAT_INT16:
                        payload_idx = mac_cell_iter * ELEMENT_PER_MAC_CELL_INT8 + 2*atom_iter;
    ...
    if (1 == payload->pd.nvdla_stripe_info.channel_end) {   // The last stripe of current channel
        ...
        if (1 == payload->pd.nvdla_stripe_info.stripe_end) {
            ...
            if (input_first_layer && input_first_channel) {
                ...
                if (DATA_FORMAT_INT8==precision)
                    assembly_sram_group_idx_working_ = assembly_sram_group_idx_available_ + 2 * atom_per_mac_cell;
                else
